...
const PeriodosDocente = () => {
    ...
    const redireccionTemplate = (rowData) => {
        return <Button icon="pi pi-chevron-circle-right" className="p-button-rounded p-button-info" onClick={() => {
            cambiarBreadLocal('Gestión de Docencia', rowData);
            navigate('/plataforma/menudocencia/'+rowData.id_Periodo);
        }}/>;
    }

    const redireccionTemplateInstructor = (rowData) => {
        return <Button icon="pi pi-chevron-circle-right" className="p-button-rounded p-button-help" onClick={() => {
            cambiarBreadLocal('Capacitaciones Impartidas', rowData);
            navigate('/plataforma/capacitacionesimpartidas/'+rowData.id_Periodo);
        }}/>;
    }

    const redireccionTemplateCapacitaciones = (rowData) => {
        return <Button icon="pi pi-chevron-circle-right" className="p-button-rounded p-button-success" onClick={() => {
            cambiarBreadLocal('Participacion en Capacitaciones', rowData);
            navigate('/plataforma/participacioncapacitaciones/'+rowData.id_Periodo);
        }}/>;
    }
    ...
    const cambiarBreadLocal = (lblActual, rowData) => {
        ...
        direccionesTemp.push({ label: actual, url: '/plataforma/gestiondocencia' });
    ...
    return <div>
        ...
        <DataTable value={periodos} responsiveLayout="scroll">
            ...
            <Column header="Gestión Docencia" body={ redireccionTemplate } style={{ width: '11%', textAlign: 'center' }}></Column>
            <Column header="Capacitaciones Impartidas" body={ redireccionTemplateInstructor } style={{ width: '15%', textAlign: 'center' }}></Column>
            <Column header="Constancias de Capacitaciones" body={ redireccionTemplateCapacitaciones } style={{ width: '15%', textAlign: 'center' }}></Column>
        </DataTable>
    </div>;
};

export default PeriodosDocente;
